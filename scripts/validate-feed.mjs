...
export function validateFeed(feed) {
  ...
  const required = ["schema_version", "generated_at", "stale_after_epoch", "source", "city", "timezone", "today", "events"];
  ...
  if (!Number.isInteger(feed.stale_after_epoch) || feed.stale_after_epoch < 0) add("stale_after_epoch must be a non-negative integer");
  ...
  if (!feed.source || typeof feed.source !== "object" || Array.isArray(feed.source)) add("source must be an object");
  else {
    ...
    if (feed.source.generated_at !== null && typeof feed.source.generated_at !== "string") add("source.generated_at must be a string or null");
    if (feed.source.attribution !== "Data: Jazz Lineup") add("source.attribution must be Data: Jazz Lineup");
  }
  ...
  if (!Array.isArray(feed.events)) add("events must be an array");
    ...
    if (typeof event.title !== "string" || !event.title.trim()) add(`${prefix}.title must be a non-empty string`);
    if (!datePattern.test(event.date ?? "")) add(`${prefix}.date must be YYYY-MM-DD`);
    if (!Array.isArray(event.sets) || !event.sets.every((set) => typeof set === "string" && timePattern.test(set))) add(`${prefix}.sets must contain 24-hour HH:MM times`);
    if (typeof event.venue_id !== "string" || !event.venue_id) add(`${prefix}.venue_id must be a non-empty string`);
    if (typeof event.venue !== "string" || !event.venue) add(`${prefix}.venue must be a non-empty string`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const input = process.argv[2] === "-" ? await new Response(process.stdin).text() : await readFile(process.argv[2] ?? "public/nyc.json", "utf8");
  let feed;
  try {
